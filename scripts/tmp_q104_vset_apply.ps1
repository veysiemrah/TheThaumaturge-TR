# q104 vset CSV'leri apply et — yalnızca q104_vset_*.csv dosyalarını işler.
...
$csvs = @(Get-ChildItem $csvDir -Filter "q104_vset_*.csv" -File) | Sort-Object Name -Unique

foreach ($csv in $csvs) {
    $name = $csv.BaseName
    $jsonPath = Join-Path $jsonRoot "$name.json"

    if (-not (Test-Path $jsonPath)) { Write-Host "[$name] json yok"; $counts.SkippedNoJson++; continue }
    ...
    if (-not $extractMatch) { Write-Host "[$name] extract yok"; $counts.SkippedNoExtract++; continue }
    ...
    } catch { Write-Host " ✗ apply" -ForegroundColor Red; $counts.Failed++; continue }

    if (-not (Test-Path $outJson)) { Write-Host " ✗ apply (json yok)" -ForegroundColor Red; $counts.Failed++; continue }
    $counts.Applied++

    & $uaGui fromjson $outJson $stagePath $mappings | Out-Null
    if (-not (Test-Path $stagePath)) { Write-Host " ✗ fromjson" -ForegroundColor Red; $counts.Failed++; continue }
    $counts.Staged++
    Write-Host " ✓" -ForegroundColor Green
}

Write-Host ""
Write-Host "Applied: $($counts.Applied), Staged: $($counts.Staged), Json yok: $($counts.SkippedNoJson), Extract yok: $($counts.SkippedNoExtract), Failed: $($counts.Failed)"
