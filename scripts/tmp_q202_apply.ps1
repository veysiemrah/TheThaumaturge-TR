# tmp_q202_apply.ps1
# q202 + q203 tüm CSV'lerini JSON'a uygula + staging'e fromjson.
...
$quests = @(
    @{ Name = "q202"; Json = "build\q202_json" },
    @{ Name = "q203"; Json = "build\q203_json" }
)
...
$allCsvs = @(Get-ChildItem "$trRoot\q202*.csv") + @(Get-ChildItem "$trRoot\q203*.csv")
...
foreach ($csv in $csvFiles) {
    $name = $csv.BaseName
    $quest = if ($name -like "q203*") { "q203" } else { "q202" }
    $jsonDir = if ($quest -eq "q203") { "build\q203_json" } else { "build\q202_json" }
