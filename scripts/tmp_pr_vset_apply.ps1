# PR vset CSV'leri apply et — yalnızca pr_vset_* / PR_vset_* dosyalarını işler.
...
$csvs = @(Get-ChildItem $csvDir -Filter "pr_vset_*.csv" -File) + @(Get-ChildItem $csvDir -Filter "PR_vset_*.csv" -File) | Sort-Object Name -Unique

foreach ($csv in $csvs) {
    $name = $csv.BaseName
    if ($name -match "^prl_") { continue } # PRL dosyalarını dışla
    $jsonPath = Join-Path $jsonRoot "$name.json"
    if (-not (Test-Path $jsonPath)) { $counts.SkippedNoJson++; continue }
    ...
    if (-not $extractMatch) { $counts.SkippedNoExtract++; continue }
